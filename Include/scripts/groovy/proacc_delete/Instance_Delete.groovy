...
class Instance_Delete {
	...
	public void user_should_navigate_to_Instance_Creation_page() {
		try {
			...
			loginfo= extent.createTest(Feature.class,"Instance deletion functionality in ProAcc");
			loginfo=loginfo.createNode(Scenario.class,"Instance deletion in ProAcc Application");

			loginfo.createNode(new GherkinKeyword("Given"), "User should navigate to Instance Creation page").pass("pass");
			Assert.assertTrue(true);
			loginfo.assignCategory("Instance Deletion")
	...
	public void user_has_to_enter_Instancename_in_search_box() {
		...
		try {
			...
			WebUI.click(findTestObject('Object Repository/ProAcc_Delete/Instance_Delete/Prj_Click'))

			loginfo.createNode(new GherkinKeyword("When"), "User has to enter Instancename in search box").pass("pass");
			Assert.assertTrue(true);
			loginfo.assignCategory("Instance Deletion")

		} catch   (Exception e){
			loginfo.createNode(new GherkinKeyword("When"), "User has to enter Instancename in search box").fail("fail").addScreenCaptureFromPath(new OnlineKeywords ().takeScreenshot(loginfo));
		}
	}
	...
	public void user_has_to_click_on_delete_button_for_Insatnce() {
		try {
			...
			WebUI.click(findTestObject('Object Repository/ProAcc_Delete/Instance_Delete/Del_Instance'))
			WebUI.click(findTestObject('Object Repository/ProAcc_Delete/Activity_Delete/Yes_Btn'))
			loginfo.createNode(new GherkinKeyword("When"), "User has to click on delete button for Insatnce").pass("pass");
			Assert.assertTrue(true);
			loginfo.assignCategory("Instance Deletion")

		} catch   (Exception e){
			loginfo.createNode(new GherkinKeyword("When"), "User has to click on delete button for Insatnce").fail("fail").addScreenCaptureFromPath(new OnlineKeywords ().takeScreenshot(loginfo));
		}
	}
	...
	public void user_has_successfully_deleted_Instance() {
		try {
			...
			loginfo.createNode(new GherkinKeyword("Then"), "User has successfully deleted Instance").pass("pass");
			Assert.assertTrue(true);
			loginfo.assignCategory("Instance Deletion")

		} catch   (Exception e){
			loginfo.createNode(new GherkinKeyword("then"), "User has successfully deleted Instance").fail("fail").addScreenCaptureFromPath(new OnlineKeywords ().takeScreenshot(loginfo));
		}
	}
}
